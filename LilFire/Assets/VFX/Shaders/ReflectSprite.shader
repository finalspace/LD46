		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
                ...
                */
                float4 color = tex2D(_MainTex, i.uv) * i.color;
                float2 offset = safemul(unity_ObjectToWorld, tex2D(_OffsetTex, i.uv) * 2 - 1);
                float4 reflColor = tex2D(_GlobalRefractionTex, i.uv + offset * _GlobalRefractionMag) * _Color;
                //reflColor = tex2D(_GlobalRefractionTex, i.uv);
                color.rgb = color.rgb * (1.0 - reflColor.a) + reflColor.rgb * reflColor.a;
