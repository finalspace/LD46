...
Shader "DDT/WaterReflection2" {
    Properties {
        [PerRendererData]_MainTex ("MainTex", 2D) = "white" {}
        _Color ("Color", Color) = (1,1,1,1)
        _NoiseTex ("NoiseTex", 2D) = "white" {}
        _NoiseSpeedX ("NoiseSpeedX", Range(-3, 3)) = -0.7
        _NoiseSpeedY ("NoiseSpeedY", Range(-3, 3)) = 1
        ...
        _ReflectionRange ("ReflectionRange", Range(0, 1)) = 0.995
        _WaveTex ("WaveTex", 2D) = "white" {}
        _WaveSpeedX ("WaveSpeedX", Range(-3, 3)) = -1.0
        _WaveSpeedY ("WaveSpeedY", Range(-3, 3)) = 1.5
        _WavePower ("WavePower", Range(0, 5)) = 1
        _Brightness("Brightness", Range(0, 5)) = 1
        _EdgeDist ("Edge Distance", Range(0, 100)) = 0.0
    ...
    SubShader {
        ...
        Pass {
            ...
            float DistToEdge(float2 uv, float size)
            {
                float2 Disc[16] =
                {
                    float2(0, 1),
                ...
                totalAlpha /= 16;
                totalAlpha = 2* totalAlpha - 1;
                totalAlpha = max(0,totalAlpha);
...
////// Emissive:
                ...
                float2 node_595 = (i.uv0+node_4707.g*float2(_NoiseSpeedX,_NoiseSpeedY)*0.1);
                float4 _WaveTex_var = tex2D(_WaveTex,TRANSFORM_TEX(node_595, _WaveTex));
                float2 node_3111 = (i.uv0+node_4707.g*float2(_WaveSpeedX,_WaveSpeedY)*0.1);
                float4 _NoiseTex_var = tex2D(_NoiseTex,TRANSFORM_TEX(node_3111, _NoiseTex));
                float3 node_3378 = float3((1.0 - ((_WaveTex_var.r*0.5+0.0)+(_NoiseTex_var.rgb.rg*_WavePower))),3.0);
                float edge = _EdgeDist > 0 ? DistToEdge(i.uv0, _EdgeDist) : 1;
                float node_9884 = (_Distortion*edge*0.2);
                ...
                float3 emissive = ((_ReflectionColor.rgb*step(_ReflectionRange,dot(float3(0,0,1),normalize(lerp(float3(0,0,1),node_3378,_Reflection))))*_ReflectionPower)+((_MainTex_var.rgb*_Color.rgb*i.vertexColor.rgb) * _Brightness));
